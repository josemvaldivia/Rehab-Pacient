...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Allows you to track the ZoomOut gesture from the Kinect.")]
	
	public class TrackZoomOut : FsmStateAction
	{
...
//		public FsmGameObject zoomedGameObj;
		...
		[Tooltip("Custom event to be sent on zoom out detection.")]
...
//			if (updateCall == PlayMakerUpdateCallType.Update)
		...
		{		
			...
			if(manager != null && KinectManager.IsKinectInitialized() && manager.GetPlayer1ID() > 0)
			{
				...
				if(!manager.IsGestureDetected(userId, KinectGestures.Gestures.ZoomOut))
				{
					manager.DetectGesture(userId, KinectGestures.Gestures.ZoomOut);
					isGestureInitialized = true;
				}

				gestureProgress.Value = manager.GetGestureProgress(userId, KinectGestures.Gestures.ZoomOut);
				
				if(gestureProgress.Value > 0.1f)
				{
					Vector3 vScreenPos = manager.GetGestureScreenPos(userId, KinectGestures.Gestures.ZoomOut);
